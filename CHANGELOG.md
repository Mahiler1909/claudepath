...
- **`claudepath status [path]` command**: report the Claude Code state attached to a single folder — transcripts, `.claude.json` entry, `history.jsonl` prompts and usage-data files — without having to scan the output of `list`. Defaults to the current directory, reads only, and exits `1` when nothing is attached so it can gate scripts
